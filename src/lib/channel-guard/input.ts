...
export const adminInput = z.discriminatedUnion("operation", [
  ...
  z.object({ operation: z.literal("settings"), chatId, waitHours: z.number().int().min(0).max(168), verification: z.boolean(), commentGate: z.boolean(), deleteJoinMessages: z.boolean().default(false), lockCommands: z.boolean().default(false), lockLinks: z.boolean().default(false), lockMedia: z.boolean().default(false), discussionChatId: chatId.nullable() }),
