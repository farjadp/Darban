...
describe("defensive admin operations", () => {
  ...
  it("supports comment gating in a directly managed supergroup", async () => {
    ...
    await saveSettings({ chatId, waitHours: 0, verification: false, commentGate: true, deleteJoinMessages: false, lockCommands: false, lockLinks: false, discussionChatId: null }, actorId);
    expect(mocks.db.guardChat.update).toHaveBeenCalledWith({ where: { id: chatId }, data: { waitHours: 0, verification: false, commentGate: true, deleteJoinMessages: false, lockCommands: false, lockLinks: false, discussionChatId: null } });
  ...
  it("supports enabling join/leave message deletion in a supergroup", async () => {
    mocks.db.chatAdmin.findUnique.mockResolvedValue({ chat: { ...chat, type: "supergroup" } });
    await saveSettings({ chatId, waitHours: 0, verification: false, commentGate: false, deleteJoinMessages: true, lockCommands: false, lockLinks: false, discussionChatId: null }, actorId);
    expect(mocks.db.guardChat.update).toHaveBeenCalledWith({ where: { id: chatId }, data: { waitHours: 0, verification: false, commentGate: false, deleteJoinMessages: true, lockCommands: false, lockLinks: false, discussionChatId: null } });
  });
  it("supports enabling slash command locking in a supergroup", async () => {
    mocks.db.chatAdmin.findUnique.mockResolvedValue({ chat: { ...chat, type: "supergroup" } });
    await saveSettings({ chatId, waitHours: 0, verification: false, commentGate: false, deleteJoinMessages: false, lockCommands: true, lockLinks: false, discussionChatId: null }, actorId);
    expect(mocks.db.guardChat.update).toHaveBeenCalledWith({ where: { id: chatId }, data: { waitHours: 0, verification: false, commentGate: false, deleteJoinMessages: false, lockCommands: true, lockLinks: false, discussionChatId: null } });
  });
  it("supports enabling link/mention locking in a supergroup", async () => {
    mocks.db.chatAdmin.findUnique.mockResolvedValue({ chat: { ...chat, type: "supergroup" } });
    await saveSettings({ chatId, waitHours: 0, verification: false, commentGate: false, deleteJoinMessages: false, lockCommands: false, lockLinks: true, discussionChatId: null }, actorId);
    expect(mocks.db.guardChat.update).toHaveBeenCalledWith({ where: { id: chatId }, data: { waitHours: 0, verification: false, commentGate: false, deleteJoinMessages: false, lockCommands: false, lockLinks: true, discussionChatId: null } });
  });
