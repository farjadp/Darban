...
type SettingsInput = { chatId: string; waitHours: number; verification: boolean; commentGate: boolean; deleteJoinMessages?: boolean; lockCommands?: boolean; lockLinks?: boolean; lockMedia?: boolean; lockForwards?: boolean; discussionChatId: string | null };
...
export async function saveSettings(input: SettingsInput, actorId: string) {
  return safe(async () => {
    ...
    if (input.discussionChatId) {
      ...
      deletion(linked.bot);
    } else if (input.commentGate || input.deleteJoinMessages || input.lockCommands || input.lockLinks || input.lockMedia || input.lockForwards) {
    ...
    return withChatLock(input.chatId, async tx => {
      ...
      const settings = { waitHours: input.waitHours, verification: input.verification, commentGate: input.commentGate, deleteJoinMessages: input.deleteJoinMessages ?? false, lockCommands: input.lockCommands ?? false, lockLinks: input.lockLinks ?? false, lockMedia: input.lockMedia ?? false, lockForwards: input.lockForwards ?? false, discussionChatId: input.discussionChatId };
